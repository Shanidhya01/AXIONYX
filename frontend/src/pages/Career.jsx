...
import { 
  Briefcase, Plus, Search, Building, 
  MapPin, Calendar, ExternalLink, 
  CheckCircle, XCircle, Clock, AlertCircle, Trash2, 
...
const Career = () => {
  ...
  const statusOptions = ['Applied', 'OA Received', 'Interview', 'Offer', 'Rejected'];

  // --- 1. FETCH DATA ---
  const fetchJobs = async () => {
      try {
          const token = localStorage.getItem('token');
          const res = await fetch(`${import.meta.env.VITE_API_BASE_URL}/api/career`, {
  ...
  const stats = {
    total: jobs.length,
    interviews: jobs.filter(j => ['Interview', 'OA Received'].includes(j.status)).length,
  ...
  const handleSaveJob = async (e) => {
    ...
    try {
        const token = localStorage.getItem('token');
        const url = isEditing ? `/api/career/${currentJobId}` : '/api/career';
  ...
  const handleDelete = async (id) => {
    if(confirm('Delete this application?')) {
        try {
            const token = localStorage.getItem('token');
            await fetch(`${import.meta.env.VITE_API_BASE_URL}/api/career/${id}`, {
                method: 'DELETE',
                headers: { 'x-auth-token': token }
            });
  ...
  const getStatusColor = (status) => {
    switch(status) {
        case 'Offer': return 'bg-green-100 text-green-600 dark:bg-green-900/30 dark:text-green-400 border-green-200 dark:border-green-800';
        case 'Rejected': return 'bg-red-100 text-red-600 dark:bg-red-900/30 dark:text-red-400 border-red-200 dark:border-red-800';
        case 'Interview': return 'bg-purple-100 text-purple-600 dark:bg-purple-900/30 dark:text-purple-400 border-purple-200 dark:border-purple-800';
        case 'OA Received': return 'bg-yellow-100 text-yellow-600 dark:bg-yellow-900/30 dark:text-yellow-400 border-yellow-200 dark:border-yellow-800';
            ...
            <table className="w-full text-left border-collapse">
                ...
                <tbody className="divide-y divide-gray-200/50 dark:divide-gray-700/50">
                    {filteredJobs.map((job) => (
                        <tr key={job._id} className="group hover:bg-white/40 dark:hover:bg-gray-800/40 transition-colors">
                            <td className="p-6">
                                <div className="flex items-center gap-3">
                                    ...
                                    <div>
                                        <div className="font-bold text-gray-800 dark:text-white">{job.company}</div>
                            ...
                            <td className="p-6">
                                <span className={`inline-flex items-center gap-1.5 px-3 py-1 rounded-full text-xs font-bold border ${getStatusColor(job.status)}`}>
                                    ...
                                    {(job.status === 'Interview' || job.status === 'OA Received') && <AlertCircle size={12} />}
                                    {job.status}
                                </span>
                            </td>
                            <td className="p-6 text-right">
                                <div className="flex items-center justify-end gap-2">
                                    {job.link && (
                                        <a href={job.link} target="_blank" rel="noreferrer" className="p-2 rounded-lg hover:bg-blue-100 dark:hover:bg-blue-900/30 text-blue-600 transition-colors">
    ...
    >
        <form onSubmit={handleSaveJob} className="space-y-4" noValidate>
            ...
            <div className="grid grid-cols-2 gap-4">
                ...
                {/* Date */}
                <div>
                    <label className="text-sm text-gray-500 mb-1 block">Date Applied</label>
                    <div className={`rounded-xl transition-all ${errors.date ? 'border border-red-500 ring-2 ring-red-500/20' : ''}`}>
                        <DatePicker 
                            value={formData.date}
                            onChange={(date) => {
            ...
            <div className="grid grid-cols-2 gap-4">
                ...
                <div>
                    ...
                    <div className="relative">
                        ...
                        >
                            ...
                            <ChevronDown size={18} className={`text-gray-500 transition-transform duration-300 ${isStatusDropdownOpen ? 'rotate-180' : ''}`} />
                        </button>
                ...
                {/* Link */}
                <div>
                    <label className="text-sm text-gray-500 mb-1 block">Job Link (Optional)</label>
                    <input 
                        type="url" 
                        placeholder="https://..." 
                        className="w-full glass-input rounded-xl dark:text-white"
            ...
            <button type="submit" className="w-full glass-btn mt-4 rounded-xl">
