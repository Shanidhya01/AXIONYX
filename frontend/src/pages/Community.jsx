...
import { 
    Search, Plus, Heart, MessageSquare, User, ChevronDown, Check, UploadCloud, File, 
    Image, Send, Loader2, ArrowRight, FileText, Download, File as FileIcon, X, Calendar, Edit, Trash2, RotateCw, AlertTriangle
} from 'lucide-react';
import { Link } from 'react-router-dom';
import Modal from '../components/UI/Modal';

// --- PostCard Component ---
const PostCard = ({ post, currentUserId, onLike, onToggleComments, onAddComment, onEditPost, onDeletePost, comments }) => {
        ...
        <div className="glass-panel p-5 rounded-2xl border-0 shadow-lg mb-4 relative">
            ...
            {post.tags.length > 0 && (
                <div className="flex flex-wrap gap-2 mb-4">
                    {post.tags.map(tag => (
                        <span key={tag} className="text-xs px-2 py-0.5 rounded-full bg-blue-100 dark:bg-blue-900/30 text-blue-600 dark:text-blue-400">#{tag}</span>
                    ))}
                </div>
            )}
            ...
            <div className="flex items-center gap-4 pt-3 border-t border-gray-100 dark:border-gray-800">
                <button onClick={() => onLike(post._id)} className={`flex items-center gap-1.5 text-sm transition-colors ${isLiked ? 'text-red-500 font-bold' : 'text-gray-500 hover:text-red-500'}`}>
                    <Heart size={18} className={isLiked ? 'fill-red-500' : ''}/> {post.likes.length}
                </button>
                <button onClick={() => onToggleComments(post._id)} className="flex items-center gap-1.5 text-gray-500 text-sm hover:text-blue-500 transition-colors">
            ...
            {post.showComments && (
                <div className="mt-4 pt-4 border-t border-gray-100 dark:border-gray-800">
                    <h4 className="font-bold text-gray-800 dark:text-white mb-3">Comments ({comments?.length || 0})</h4>
                    
                    {/* Comment List */}
                    <div className="max-h-60 overflow-y-auto space-y-3 pr-2 custom-scrollbar">
                        {comments?.map(comment => (
                            <div key={comment._id} className="p-3 bg-gray-50 dark:bg-gray-800 rounded-lg">
                                <p className="text-xs font-bold text-gray-800 dark:text-white mb-1">{comment.author.name}</p>
                                <p className="text-sm text-gray-700 dark:text-gray-300">{comment.content}</p>
                            </div>
                        ))}
                        {comments?.length === 0 && <p className="text-xs text-gray-500 text-center py-2">Be the first to comment!</p>}
                    </div>
                    ...
                    <form onSubmit={handleCommentSubmit} className="flex gap-2 mt-4">
                        <input 
                            ...
                            ref={commentInputRef}
                        />
                        <button type="submit" className="p-2.5 bg-blue-600 text-white rounded-xl hover:bg-blue-700 active:scale-95">
                            <Send size={18} />
                        </button>
                    </form>
                </div>
...
const Community = () => {
    ...
    const [postForm, setPostForm] = useState({ title: '', content: '', tags: '', imageFile: null, imageUrl: '' });
    const [postError, setPostError] = useState('');
    const [contributors, setContributors] = useState([]);
    const imageInputRef = useRef(null);
    const [isRefreshing, setIsRefreshing] = useState(false);

    const categories = ['All', 'My Posts', 'ExamHelp', 'LostFound', 'Events', 'StudyGroup'];
    ...
    const fetchAllData = async (silent = false) => {
        ...
        try {
            const [userRes, postRes, contributorsRes] = await Promise.all([
                ...
                fetch(`${import.meta.env.VITE_API_BASE_URL}/api/community/posts`, { headers: { 'x-auth-token': token } }),
                fetch(`${import.meta.env.VITE_API_BASE_URL}/api/community/top-users`, { headers: { 'x-auth-token': token } })
            ]);

            const userData = await userRes.json();
            const postData = await postRes.json();
            const contributorData = await contributorsRes.json();

            // CRITICAL: Store user ID as string immediately
            setCurrentUserId(userData._id.toString()); 
            
            // Map new post data to maintain existing 'showComments' state
            const updatedPosts = postData.map(newPost => {
                const existingPost = posts.find(p => p._id === newPost._id);
                return { 
                    ...newPost, 
                    // CRITICAL: Ensure likes array stores IDs as strings for comparison
    ...
    const handleImageChange = (e) => {
        ...
        if (file) {
            ...
            reader.onloadend = () => {
                setPostForm(prev => ({ ...prev, imageUrl: reader.result, imageFile: file })); 
            };
    ...
    const handleFormSubmit = async (e) => {
        ...
        const url = isEditing ? `/api/community/posts/${editingPost._id}` : '/api/community/posts';
        ...
        try {
            ...
            if (!res.ok) throw new Error(responseData.msg || `Failed to ${isEditing ? 'update' : 'create'} post`);

            // Full refresh to ensure updated data is correctly merged
            await fetchAllData(true); 
            
            // Clear form and reset states
            setPostForm({ title: '', content: '', tags: '', imageFile: null, imageUrl: '' });
            setEditingPost(null);
    ...
    const confirmDeletion = async () => {
        ...
        try {
            const res = await fetch(`${import.meta.env.VITE_API_BASE_URL}/api/community/posts/${postId}`, { 
                method: 'DELETE', 
    ...
    const handleToggleLike = async (postId) => {
        ...
        const token = localStorage.getItem('token');
        try {
            const res = await fetch(`${import.meta.env.VITE_API_BASE_URL}/api/community/posts/${postId}/like`, { 
                method: 'PUT', 
                headers: { 'x-auth-token': token } 
            });
    ...
    const fetchComments = async (postId) => {
        const token = localStorage.getItem('token');
        try {
            const res = await fetch(`${import.meta.env.VITE_API_BASE_URL}/api/community/posts/${postId}/comments`, { 
                headers: { 'x-auth-token': token } 
            });
            const commentsData = await res.json();
            setAllComments(prev => ({ ...prev, [postId]: commentsData }));
        } catch (err) {
    ...
    const finalFilteredPosts = posts.filter(post => {
        ...
        const matchesCurrentTag = filterTag === 'My Posts' ? matchesMyPosts : matchesTag;
    ...
    return (
        ...
        <div className="space-y-8 animate-in fade-in duration-500 pb-10">
            ...
            <div className="flex flex-col md:flex-row justify-between items-start md:items-center gap-4">
                ...
                <div className="flex gap-4">
                    ...
                    <button 
                        onClick={() => { setIsCreateOpen(true); setEditingPost(null); setPostForm({ title: '', content: '', tags: '', imageFile: null, imageUrl: '' }); }}
                    ...
                    <div className="flex flex-col md:flex-row gap-4 glass-panel p-4 rounded-xl shadow-md">
                        ...
                        <div className="flex gap-2 overflow-x-auto p-2 no-scrollbar border border-gray-200/50 dark:border-gray-700/50 rounded-xl">
                            {categories.map(cat => (
                                ...
                                >
                                {cat === 'All' ? 'All' : cat === 'My Posts' ? 'My Posts' : `#${cat}`}
                                </button>
                            ))}
                        </div>
                    </div>
                        ...
                        <div className="space-y-4">
                            ...
                            {finalFilteredPosts.length === 0 && (
                                <div className="py-20 text-center text-gray-400 glass-panel rounded-2xl">
                                    ...
                                    <p className="text-sm mt-1">Try changing the filter or making a new post!</p>
                                </div>
                            )}
                        </div>
                    )}
                ...
                <div className="lg:col-span-1 space-y-6">
                    ...
                    <div className="glass-panel p-5 rounded-2xl border-0 shadow-lg">
                        <h3 className="font-bold text-gray-800 dark:text-white mb-2 flex items-center gap-2">
                            <FileText size={20} className="text-green-500"/> Resource Repository
                        </h3>
                        <p className="text-sm text-gray-500 dark:text-gray-400 mb-4">
                            Access student notes, past papers, and lab manuals shared by your peers.
                        </p>
                        <Link 
                            to="/resources" 
                        ...
                        <div className="space-y-4">
                            {contributors.length > 0 ? (
                                contributors.map((c, index) => (
                                    <div key={c.name} className="flex items-center gap-3 opacity-80">
                                        ...
                                        <div className="flex-1">
                                            ...
                                            <div className="text-xs text-gray-500">{c.posts} Posts, {c.likes} Likes</div>
                                        </div>
                                ...
                                <p className="text-sm text-gray-500">No recent contributions.</p>
                            )}
                        </div>
                    </div>
                </div>
        ...
        >
            <form onSubmit={handleFormSubmit} className="space-y-4" noValidate>
                ...
                {postError && <div className="p-3 bg-red-500/10 border border-red-500/20 text-red-600 rounded-xl text-sm font-bold">{postError}</div>}
                
                {/* TITLE INPUT */}
                <div>
                    <label className="text-sm text-gray-500 mb-1 block">Post Title (Required)</label>
                    <input 
                        type="text" 
                ...
                <div>
                    ...
                    <textarea 
                        ...
                        maxLength={1000}
                    />
                    <p className="text-xs text-gray-500 text-right mt-1">{postForm.content.length}/1000</p>
                </div>
                ...
                <div className="flex gap-4 items-center">
                    ...
                    >
                        <Image size={18} /> {postForm.imageUrl ? "Change Image" : "Add Photo"}
                    </button>
                    {postForm.imageUrl && (
                        <div className="relative w-20 h-20 rounded-xl overflow-hidden shadow-md">
                            ...
                            <button type="button" onClick={() => setPostForm(prev => ({...prev, imageUrl: '', imageFile: null}))} className="absolute top-1 right-1 p-0.5 bg-red-500 text-white rounded-full"><X size={12}/></button>
                        </div>
                    )}
                </div>
                ...
                {/* Tags */}
                <div>
                    <label className="text-sm text-gray-500 mb-1 block">Tags (e.g., ExamHelp, Event, LostFound)</label>
                    <input 
                        type="text" 
                ...
                >
                    {isSubmitting ? (
                        <Loader2 size={20} className="animate-spin mx-auto" /> // Show spinner
                    ) : (
                        editingPost ? "Save Changes" : "Post to Feed"
                    )}
                </button>
            </form>
        </Modal>
        ...
        <Modal isOpen={isDeleteModalOpen} onClose={() => setIsDeleteModalOpen(false)} title="Confirm Deletion">
            <div className="text-center space-y-6 p-4">
                ...
                <h3 className="text-lg font-bold text-gray-800 dark:text-white">Are you sure you want to delete this post?</h3>
                <p className="text-gray-500 dark:text-gray-400 text-sm">
                    This action is permanent and cannot be undone. All comments will also be removed.
                </p>
